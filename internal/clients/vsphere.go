...
import (
	...
	"github.com/giattal/provider-jet-vsphere/apis/v1alpha1"
)
...
const (
	...
	// VSPHERE credentials environment variable names
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		ps.Configuration = map[string]interface{}{
			"host": vsphereCreds[keyHost],
		}
		// set environment variables for sensitive provider configuration
		ps.Env = []string{
			fmt.Sprintf(fmtEnvVar, envUsername, vsphereCreds[keyUsername]),
			fmt.Sprintf(fmtEnvVar, envPassword, vsphereCreds[keyPassword]),
		}
		return ps, nil
	}
}
